...
export const createUser = async (user: Pick<IUser, "username" | "email" | "password">) => {
    return await User.create({ username: user.username, email: user.email, password: user.password });
}
...
export const getUsersStats = async () => {
    return await User.aggregate([
        { $lookup: { from: "histories", localField: "_id", foreignField: "userId", as: "userHistory" } },
