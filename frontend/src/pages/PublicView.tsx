...
import { FiAlertCircle, FiArrowLeft, FiBarChart2, FiCalendar, FiClock, FiCode, FiGrid, FiHome, FiSearch, FiShare2, FiTable } from "react-icons/fi";
...
const PublicView = () => {
  ...
  const [historyData, setHistoryData] = useState<PublicViewData | null>(null);
  const [viewMode, setViewMode] = useState<"table" | "card" | "chart" | "tree" | "graph">("table");
  ...
  useEffect(() => {
    const fetchPublicData = async () => {
      try {
        ...
        if (res.ok) {
          setHistoryData(data);
          setViewMode((data.type ?? "table") as "table" | "card" | "chart" | "tree" | "graph");
        } else {
  ...
  const viewModes: Array<{ id: ViewMode; icon: ComponentType<{ size?: number }>; label: string }> = [
