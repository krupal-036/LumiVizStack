...
export default function SignUp() {
  ...
  const validateAll = () => {
    ...
    Object.keys(formData).forEach((key) => {
      const error = getFieldError(key as keyof RegisterFormData, formData[key as keyof RegisterFormData]);
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      showAlert(err.message || "Connection failed. Check if the server is running.", "Server Error", 1);
      clearError(3);
    } finally {
      setIsLoading(false);
    }
