...
export default function Login() {
    ...
    const getFieldError = (name: keyof LoginFormData, value: string) => {
    ...
    const handleChange = (e: ChangeEvent<HTMLInputElement>) => {
        ...
        if (value.length > 0) {
            const fieldError = getFieldError(name as keyof LoginFormData, value);
            setErrors((prev) => ({
                ...prev,
                [name]: fieldError,
            }));
    ...
    const validateAll = () => {
        ...
        Object.keys(formData).forEach((key) => {
            ...
            const error = getFieldError(fieldKey, fieldValue);
    ...
    const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            if (contentType && contentType.includes("application/json")) {
                ...
                showAlert(
                    text || "Server not Available. Please try again later.",
                    "Server Error",
                    1,
                );
            }
            ...
            if (!response.ok) {
                const errorMessage = String(data?.message || data?.error || "Invalid credentials.");
            ...
            showAlert(err.message || "Unable to connect to server.", "Server Error");
            clearError(3);
        } finally {
            setIsLoading(false);
        }
