    ...
    <AlertContext value={contextValue}>
      ...
      <div className="fixed bottom-8 left-0 right-0 z-[9999] px-6 flex flex-col items-center pointer-events-none">
        ...
        {/* {alerts.length > 2 && (
          <button
            onClick={clearAll}
            // Added mt-4 to give space from the stack and z-50 to stay on top
            className="mt-4 z-50 pointer-events-auto flex items-center gap-2 px-4 py-2 rounded-full bg-white/10 hover:bg-white/20 backdrop-blur-md border border-white/10 text-xs font-medium text-slate-400 transition-all hover:text-slate-200 animate-in fade-in zoom-in"
          >
...
const AlertPopup = ({ alert, onClose, index, total }) => {
    ...
    <div style={stackStyle} className="pointer-events-none">
      ...
      `}>
        ...
        <div className="flex-1 min-w-0 pr-2 py-4 sm:py-0">
          ...
          <p className="text-[13px] opacity-70 font-medium leading-relaxed truncate">{alert.message}</p>
        </div>
