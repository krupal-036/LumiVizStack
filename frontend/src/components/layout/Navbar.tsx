...
export default function Navbar() {
    ...
    return (
        ...
        >
            <div
                className={`mx-auto max-w-screen-2xl z-70 px-0 md:px-2 transition-all duration-300`}
            >
                <div
                    className={`flex items-center justify-between px-4 h-16 rounded-2xl border transition-all duration-300 ${
                        scrolled
                            ? menuOpen
                                ? "bg-white/80 dark:bg-slate-900/80 z-70 border-slate-200 dark:border-slate-800 shadow-lg"
                                : "backdrop-blur-md bg-white/80 dark:bg-slate-700/50 z-70 border-slate-200 dark:border-slate-800 shadow-lg"
                ...
                >
                    ...
                    <div className="flex items-center gap-1 z-70 sm:gap-3">
                        {user && credits !== null && user.role !== "admin" && (
                            <div
                                className={`flex flex-row items-center justify-center gap-1 sm:gap-2 px-3 py-1.5 rounded-full border text-xs font-bold transition-all duration-300 ${
                                    credits === 0
                                ...
                                title={
                                    credits === 0
                                        ...
                                        : `You've got ${credits || 0} credits left to use today!`
                                }
                            >
                                <FiZap
                                    size={14}
                                ...
                                >
                                    {credits ?? 0}
                                    <span
                                        className={`hidden sm:inline ml-0.5 ${credits === 0 ? "text-slate-400" : "text-indigo-500"}`}
                                    >
                            ...
                            {user ? (
                                <div className="flex items-center gap-4">
                                    ...
                                    <button
                                        ...
                                        title={`See you soon, ${user.name}! Ready to sign out?`}
                                ...
                                <>
                                    ...
                                    <Link
                                        to="/register"
                                        className="px-5 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-bold shadow-lg shadow-indigo-500/25 transition-all active:scale-95"
                                    >
                                        Get Started
                                    </Link>
                                </>
                        ...
                        {user ? (
                            ...
                            <div className="grid grid-cols-2 gap-4 mt-4">
                                <Link
                                    to="/login"
                                    onClick={() => setMenuOpen(false)}
                                    className="p-4 rounded-2xl bg-slate-100 dark:bg-slate-800 text-center font-bold"
                                >
                                    Log In
                                </Link>
                                <Link
                                    to="/register"
                                    onClick={() => setMenuOpen(false)}
                                    className="p-4 rounded-2xl bg-indigo-600 text-white text-center font-bold"
                                >
                                    Sign Up
                                </Link>
                            </div>
