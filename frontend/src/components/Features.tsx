...
export const renderValue = (
    val: any,
    path: any,
    forceImages: Record<any, any>,
    setForceImages: Dispatch<SetStateAction<Record<any, any>>>,
) => {
    ...
    if (typeof val === "boolean")
        return <span className="font-mono text-blue-600 dark:text-blue-400">{val.toString()}</span>;
    ...
    if (val.startsWith("http")) {
            ...
            return (
                <div className="mt-2 group relative inline-block">
                    ...
                    <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="absolute top-1 right-1 p-1 bg-white dark:bg-gray-800 rounded-full opacity-0 group-hover:opacity-100 transition-opacity"
                    >
        ...
        if (isVideo)
            return (
                <video controls className="max-w-[250px] rounded mt-2 shadow-sm">
                    <source src={val} />
                </video>
            );
        if (isAudio)
        ...
        return (
            <div className="flex items-center gap-2">
                <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="text-blue-500 hover:underline text-xs truncate max-w-[120px]"
                >
                    Link
                </a>
                <button
