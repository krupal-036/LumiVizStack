            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
                {data.map((item, idx) => {
                    ...
                    return (
                        <div
                            key={idx}
                            className="group flex flex-col h-full bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-xl hover:-translate-y-1.5 transition-all duration-300 overflow-hidden"
                        >
                            {imgSrc && (
                                    ...
                                    {Object.entries(item).map(([key, val], entryIdx) => {
                                        ...
                                        return (
                                            <div key={key} className="flex flex-col gap-1">
                                                ...
                                                <div
                                                    className={`break-words leading-relaxed ${entryIdx === 0 && !imgSrc ? "text-lg font-bold text-slate-900 dark:text-white" : "text-slate-700 dark:text-slate-300 text-sm"}`}
                                                >
                                                    <SmartCell
                                                        value={val}
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-8 animate-in fade-in duration-300">
                    ...
                    <div className="relative max-w-5xl w-full h-full flex flex-col items-center justify-center animate-in zoom-in-95 duration-300">
                        <div className="absolute top-0 right-0 p-4 flex gap-3 z-50">
                            ...
                            <button
                                onClick={() => setPreviewImage(null)}
                                className="p-3 bg-white/10 hover:bg-red-500 rounded-full text-white backdrop-blur-xl transition-all"
                            >
                                <FiX size={20} />
                            </button>
                        </div>
