...
const getLayoutedElements = (nodes: any, edges: any, direction = "LR") => {
    ...
    nodes.forEach((node: any) => dagreGraph.setNode(node.id, { width: 180, height: 40 }));
...
const FlowTreeView = ({ data = {} }: { data?: any }) => {
    ...
    useEffect(() => {
        ...
        const traverse = (obj: any, parentId = null) => {
            ...
            ${
                ...
                if (isParent) traverse(value, id as any);
            });
        };
    ...
    return (
        <div className="group relative w-full h-[600px] rounded-[2rem] border border-slate-200 dark:border-zinc-800 bg-slate-50 dark:bg-zinc-950 shadow-2xl overflow-hidden">
            ...
            >
                ...
                <Panel
                    position="top-right"
                    className="flex gap-2 p-2 bg-white/80 dark:bg-zinc-900/80 backdrop-blur-xl border border-slate-200 rounded-2xl shadow-xl"
                >
                    <button
                        onClick={() => onLayout("LR")}
