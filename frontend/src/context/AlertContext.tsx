    ...
    const typeStyles = useMemo(() => {
        ...
        return {
            bg: {
                ...
                default:
                    "bg-slate-50/90 border-slate-200 text-slate-900 dark:bg-slate-900/80 dark:border-white/10 dark:text-slate-200",
            },
            ping: {
                ...base,
                1: "bg-red-400",
      ...
      `}
            >
                <div className="flex-shrink-0 relative flex items-center justify-center w-5 h-5 ml-4 sm:ml-0">
                    <span
                        className={`absolute inline-flex h-full w-full rounded-full opacity-20 animate-ping ${typeStyles.ping[alert.type] || typeStyles.ping.default}`}
                    ></span>
                    <span
                        className={`relative inline-flex rounded-full h-3 w-3 ${typeStyles.dot[alert.type] || typeStyles.dot.default}`}
                    ></span>
                </div>
                ...
                <div className="flex-shrink-0 flex items-center self-center pt-0.5 border-l border-gray-100 dark:border-white/5 pl-2 ml-1 mr-2 sm:mr-0">
                    <button
                        onClick={onClose}
                        className="p-1.5 rounded-lg transition-all duration-200 text-gray-500 hover:text-gray-600 hover:bg-gray-100/80 dark:hover:bg-white/10 active:scale-90"
                    >
                ...
                <div className="absolute bottom-0 left-0 right-0 h-[3px] bg-gray-100/50 dark:bg-white/5">
                    <div
                        className={`h-full animate-toast-progress origin-left ${typeStyles.bar[alert.type] || typeStyles.bar.default}`}
                    />
                </div>
            </div>
        </div>
